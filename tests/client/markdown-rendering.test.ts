...
describe('MarkdownRenderer', () => {
  it('waits for deferImages to be lifted before requesting its published image', async () => {
    const resolver = vi.fn(() => '/api/studio/attachments/ROOM1/answer.png')
    ...
    expect(wrapper.get('img').attributes('src')).toBe('/api/studio/attachments/ROOM1/answer.png')
    wrapper.unmount()
  })
  it('uses the custom image resolver for local images while leaving public images alone', () => {
    const resolveImageUrl = vi.fn(path => `/api/studio/attachments/ROOM1/${encodeURIComponent(path.split('/').pop())}`)
    const wrapper = mount(MarkdownRenderer, { props: {
      content: '![图片](</workspace/马年 image.png>)\n\n![public](https://example.com/photo.png)', resolveImageUrl,
    } })
    const images = wrapper.findAll('img')
    expect(images[0].attributes('src')).toBe('/api/studio/attachments/ROOM1/%E9%A9%AC%E5%B9%B4%20image.png')
